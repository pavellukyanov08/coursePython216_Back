...
# 1) в присвоении задачи пользователю вместо имени пользователя (ключа) из словаря выводится значения email, role и задачи, которые тоже являются ключами.
# 2) в mark_task_completed проверяется, если задача существует, то is_completed=True. В print_tasks происходит итерация по списку задач и из-за условия "if self.is_completed" статус "выполнена " назначается всем задачам.
# Подскажите, как можно исправить данные недочеты? Задача сама по себе несложная, но взаимодействие со всеми списками немного путает.
